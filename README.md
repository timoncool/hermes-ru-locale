...
> ⚠️ **Бета-версия.** Этот перевод я делал для себя как тестовый проект для Hermes Agent. Всё работает, но могут быть шероховатости — если что, ваш Hermes сам разберётся.
...
3. Hermes снова работает, русский язык на месте — перезапускать `install.ps1` не нужно
